package com.example.geyingqi.blog.view;
...
public class PullScrollView extends ScrollView {
    ...
    //ScrollView 的content View
    private View mContentView;

    //ScrollView的ContentView矩形
    ...
    private void doActionMove(MotionEvent event){
        //当滚动到顶部的时候,将设置状态为正常,避免先向上拖动再向下拖动到顶端后首次接触不相应的问题
        if (getScrollY() == 0){
            mState = State.NORMAL;
            //滑动经过初始位置的时候,修正TouchDown的坐标为当前touch点的坐标
            if(isTop){
                isTop = false;
        ...
        if (isMoving){
            ...
            //计算header的移动距离(手势移动的距离*阻尼系数*0.5)
